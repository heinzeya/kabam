...
if(whoAmI.name === 'kabam'){
  ...
  var fileprefix = __dirname+'/node_modules/';
} else {
...
//  kabam.usePlugin(require('kabam-plugin-welcome'));
  kabam.usePlugin(require(fileprefix+'kabam-plugin-my-profile'));

//private messages api
  kabam.usePlugin(require(fileprefix+'kabam-plugin-private-message'));

//geoip
  kabam.usePlugin(require(fileprefix+'kabam-plugin-geoip'));

//enable plugin to send emails
  if (config.emailConfig) {
    kabam.usePlugin(require(fileprefix+'kabam-plugin-notify-email'));
  }

  //rest api for mongoose models
  kabam.usePlugin(require(fileprefix+'kabam-plugin-rest'));

  //rest api for mongoose models
  kabam.usePlugin(require(fileprefix+'kabam-plugin-users'));

  kabam.usePlugin(require(fileprefix+'kabam-core-web-frontend'));
  ...
  kabam.extendApp(function(core){
    ...
    core.app.locals.mincerENV.prependPath(__dirname+'public/bower_components/kabam-core-web-frontend/public/components');
    ...
    core.app.locals.mincerENV.appendPath(__dirname+'public/bower_components/kabam-core-web-frontend/public');
  });
  ...
  if(config.logs){
    if(config.logs.filelog===true){
      kabam.usePlugin(require(fileprefix+'kabam-plugin-logger-file'));
    }
    if(config.logs.httpMongo===true){
      kabam.usePlugin(require(fileprefix+'kabam-plugin-logger-http-mongo'));
    }
    if(config.logs.errorMongo===true){
      kabam.usePlugin(require(fileprefix+'kabam-plugin-logger-error-mongo'));
    }
  }
  return kabam;
};
